        ...
        <div className="grid gap-8 sm:gap-12 md:grid-cols-3 lg:gap-16">
          <div className="flex flex-col gap-6 sm:gap-8 items-center md:items-start">
            ...
            <div className="flex gap-2">
              ...
              <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded hover:opacity-90" aria-label="LinkedIn" style={{ backgroundColor: colors.primary, color: colors.secondary }}><LinkedInIcon size={16} /></a>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:gap-6">
            ...
            <form onSubmit={handleSignup} className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email Address" required className="border-b px-0 py-2 text-sm bg-transparent flex-1" style={{ borderColor: 'rgba(26,26,26,0.3)', color: colors.foreground }} />
              <button type="submit" disabled={loading} className="w-full sm:w-fit rounded-lg px-5 sm:px-6 py-2.5 sm:py-3 text-sm font-semibold lowercase transition-colors hover:opacity-90" style={{ backgroundColor: colors.primary, color: colors.secondary }}>{loading ? 'signing up...' : 'sign up'}</button>
            </form>
          </div>
        </div>
