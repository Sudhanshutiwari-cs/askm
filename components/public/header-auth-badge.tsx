  ...
  if (!mounted) {
    if (isMobile) {
      return (
        <Link
          href="/patient-login"
          className="rounded-lg px-6 py-3 text-sm font-semibold lowercase tracking-wide transition-all border text-center block"
    ...
    return (
      <Link
        href="/patient-login"
        className="rounded-lg px-3.5 xl:px-4 py-2 xl:py-2.5 text-xs sm:text-sm font-semibold lowercase tracking-wide transition-all border hover:bg-black/5 whitespace-nowrap"
        style={{ borderColor: primaryColor, color: primaryColor }}
      >
        patient login
      </Link>
  ...
  if (user) {
      ...
      return (
        <div className="flex flex-col gap-2">
          <Link
            href={portalHref}
            onClick={onNavigate}
            className="rounded-lg px-4 py-3 text-sm font-semibold lowercase tracking-wide transition-all border flex items-center justify-between shadow-sm"
            style={{
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <div className="text-left leading-tight">
                <p className="font-bold lowercase">{firstName}</p>
    ...
    return (
      <div className="flex items-center gap-2">
        <Link
          href={portalHref}
          className="rounded-lg px-3 xl:px-4 py-1.5 xl:py-2 text-xs sm:text-sm font-semibold lowercase tracking-wide transition-all border hover:shadow-sm flex items-center gap-2 whitespace-nowrap group"
          style={{
            borderColor: primaryColor,
            backgroundColor: `${primaryColor}0d`,
            color: primaryColor,
          }}
          title={`Logged in as ${firstName} (${user.email}). Click to view dashboard`}
        >
          <div
          ...
          <span className="font-medium lowercase group-hover:underline">
            {firstName}
          </span>
        </Link>
        <button
